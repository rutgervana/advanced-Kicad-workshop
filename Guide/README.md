...
## Schematic Notes 
Open the project `Kicad_workshop_volundr_clock.zip`{:.python} given  with this guide, and use the unarchive function of Kicad. You can browse the schematic to view which components we will be using in our design. 
...
We will be using the `GetFootprint()`{:.python} function, which needs `'wxPoint' aPosition`, `'PCB_LAYER_ID' aActiveLayer` and `'bool' aVisibleOnly`{:.python} as arguments.
So first save the position as a `wxPoint`{:.python} which uses the internal Kicad reference of nanometers. 
We can use two methods to transform from mm to nm:`FromMM()`{:.python} and `wxPointMM()`{:.python} which coverses both x and y in one command. We will use the last one.
...
```
To get the `LayerID`{:.pyhton} we will use the function `GetLayerID('wxString' aLayerName)`{:.python} which takes the the layername given in the editor as argument. 
The last argument we just set as `True`{:.python}, because our ESP8266 module is visible in the editor.
...
PCBnew must be given a sign that the display must be refreshed by running `Refresh()`{:.python}
## Making the script
...
- Make a file `place_leds.py`{:.python} in save it in an path without spaces(for windows make a direcroy on C:blabla).
...
- Paste the following in the scripting console `exec(open("absolute path to script").read())`{:python}(Windows: `exec(open("C://blabla/place_leds.py").read())`{:.python}
...
Getting al the needed footprints by position is not very practical so we want to get them by reference with the function `FindModuleByReference()`{:.python}.
Note: there is a discrepancy between the Python API and the implementation, so `FindModuleByReference()`{:.python} is used instead of `FindFootprintByReference()`{:.python}
...
Tip: to get the position of a pad the `GetCenter()`{:.python} function can be used.
...
An Openscad script `copper_fills.scad`{:.python} is used to generate the copper fill geometry. Due to the limitations on how the copper fill is generated, the outer ring needs a small gap.
The Plugin is also located in the project folder as `DXFZonePlugin`{:.python} to install it copy it to the Plugins Folder.
The location of this folder can be found in pcbnew by Preferencs &rarr Configure Paths `KICAD_PLUGIN_DIR`{:.python}.
